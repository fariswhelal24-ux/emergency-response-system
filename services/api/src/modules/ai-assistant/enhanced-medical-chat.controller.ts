...
export const enhancedMedicalChatController = {
   ...
   */
  async getMedicalAdvice(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      // Get medical advice
      const advice: MedicalAdvice = await BilinguMedicalAdvisor.getMedicalAdvice(
        message,
        history,
   ...
   */
  async clarifySituation(req: AuthRequest, res: Response): Promise<void> {
    try {
   ...
   */
  async handleEmergencyInquiry(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const advice: MedicalAdvice = await BilinguMedicalAdvisor.getMedicalAdvice(urgentMessage, history);
      ...
      res.json({
        ...
        data: {
          ...
          fullAdvice: advice,
          language: advice.language,
          emergencyNumbers: advice.language === "ar" 
   ...
   */
  async getMedicalResources(req: AuthRequest, res: Response): Promise<void> {
    try {
      const userId = req.user?.id;
   ...
   */
  async checkSymptoms(req: AuthRequest, res: Response): Promise<void> {
    try {
      const userId = req.user?.id;
